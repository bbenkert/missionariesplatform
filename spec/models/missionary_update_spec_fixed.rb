...
RSpec.describe MissionaryUpdate, type: :model do
  ...
  describe 'Associations' do
    ...
    it { should have_one(:missionary_profile).through(:user) }
  end
  ...
  describe 'Validations' do
    ...
    it { should define_enum_for(:update_type).with_values(
      general_update: 0,
      prayer_request: 1,
      praise_report: 2,
      ministry_news: 3
    ...
    context 'prevents overly long titles' do
      it 'is invalid with titles longer than 200 characters' do
        update = build(:missionary_update, user: user, title: 'x' * 201)
        expect(update).not_to be_valid
        expect(update.errors[:title]).to include("is too long (maximum is 200 characters)")
      end
    end
  end
  ...
  describe "Scopes" do
    ...
    describe ".recent" do
      it "orders updates by creation date descending" do
        expect(MissionaryUpdate.recent.first).to eq(recent_update)
      end
    end
  ...
  describe "Workflow and Status" do
    ...
    it "starts as published by default" do
      expect(update.status).to eq("published")
    end
  ...
  describe "Edge Cases and Error Handling" do
    ...
    it "handles special characters in content" do
      special_content = "Hello 世界! 🌟 Special chars: àáâãäåæ"
      update = create(:missionary_update, user: user, content: special_content)
      expect(update.content.to_plain_text).to include("世界")
    end
