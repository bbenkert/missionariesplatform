...
RSpec.describe 'Missionary Search and Filter', type: :system do
  ...
  it 'displays correct Open Graph and canonical tags on missionary profile page' do
    ...
    visit missionary_path(missionary_with_avatar)

    # Verify Open Graph tags
    expect(page).to have_css('meta[property="og:title"][content="Avatar Missionary"]', visible: false)
    expect(page).to have_css('meta[property="og:description"][content="This is a test bio for Open Graph tags."]', visible: false)
    expect(page).to have_css("meta[property=\"og:image\"][content*=\"#{missionary_with_avatar.avatar.filename}\"]", visible: false)
    expect(page).to have_css("meta[property=\"og:url\"][content=\"#{missionary_url(missionary_with_avatar, host: 'www.example.com', port: 80)}\"]", visible: false)
